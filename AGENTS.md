...
- **Deployment**: Static build served via Nginx webserver
- **Animation Libraries**: Framer Motion, GSAP (for complex sequences)

This is a **static portfolio site** - no backend, no API routes, no database. All content is hardcoded or loaded from static JSON/markdown files.

## Design System

### Primary Accent Color
- **Aquamarine**: `#7fffd4` (HSL: 160, 100%, 75%)
- Use for CTAs, highlights, hover states, glows, and accent elements

### Visual Style: Apple Liquid Glass
- Glassmorphism with backdrop-blur effects
- Subtle transparency and layered depth
- Smooth, fluid animations everywhere
- Soft shadows and glows using the accent color
- Dark mode as the PRIMARY theme (light mode optional)

### Animation Philosophy
- **ANIMATE EVERYTHING** - Every interaction should feel alive
...
- Liquid/morphing shapes in backgrounds
...
- **Structured data**: Add JSON-LD for Person schema (portfolio owner)
...
- NO backend logic - this is a static site served by Nginx
- NO API routes or server functions
- NO database connections
- Content comes from static files (JSON, markdown, or hardcoded)
- Forms should use external services (Formspree, EmailJS, etc.) or mailto links

- Assume users want to discuss and plan rather than immediately implement code.
- Before coding, verify if the requested feature already exists. If it does, inform the user without modifying code.
- For debugging, check browser console and build output.
...
5. ASK CLARIFYING QUESTIONS: If any aspect of the request is unclear, ask for clarification BEFORE implementing. Wait for their response before proceeding and calling tools. You should generally not tell users to manually edit files or provide data such as console logs if you can do that yourself.
...
- Use toast components sparingly - only for critical user feedback.
- Prioritize smooth animations using Framer Motion for all transitions.
- Use GSAP for complex animation sequences (timelines, ScrollTrigger).
- Every interactive element should have hover/focus animations.
...
/* Accent glow */
.glow {
  box-shadow: 0 0 40px rgba(127, 255, 212, 0.3);
}
```
...
- Leverage the index.css and tailwind.config.js files to create a consistent design system that can be reused across the app instead of custom styles everywhere.
- Create variants in the components you'll use. Shadcn components are made to be customized!
- You review and customize the shadcn components to make them look beautiful with the correct variants.
- CRITICAL: USE SEMANTIC TOKENS FOR COLORS, GRADIENTS, FONTS, ETC. It's important you follow best practices. DO NOT use direct colors like text-white, text-black, bg-white, bg-black, etc. Everything must be themed via the design system defined in the index.css and tailwind.config.js files!
- Always consider the design system when making changes.
- Pay attention to contrast, color, and typography.
- Always generate responsive designs.
- Beautiful designs are your top priority, so make sure to edit the index.css and tailwind.config.js files as often as necessary to avoid boring designs and leverage colors and animations.
...
- ALWAYS use HSL colors in index.css and tailwind.config.js
...
- When implementing:
  ...
  - Edit the `tailwind.config.js` and `index.css` based on the design ideas or user requirements.  Create custom variants for shadcn components if needed, using the design system tokens. NEVER use overrides. Make sure to not hold back on design.
  ...
  // First enhance your design system, then:
    // Beautiful by design
   - Images can be great assets to use in your design. Great for hero images, banners, etc. You prefer generating images over using provided URLs if they don't perfectly match your design. You do not let placeholder images in your design, you generate them. You can also use the web search tool to find images about real people or facts for example.
...
- You go above and beyond to make the user happy. The MOST IMPORTANT thing is that the app is beautiful and works. That means no build errors. Make sure to write valid JavaScript or TypeScript and CSS code following the design system. Make sure imports are correct.
